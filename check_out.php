<?php include_once 'config.php'; 
								...
								if(isset($_POST['id'])){
                                    $address=$_POST['id'];
                                    $result="";
									$diachi=mysqli_query($conn,"SELECT * FROM `tbl_huyen` WHERE (`id`='$address')");
                               if(mysqli_num_rows($diachi)>0){
                                    
                                    while($rows=mysqli_fetch_array($diachi))
                                    {
                                        $result.='<option value="'.$rows['maHuyen'].'" >'. $rows['tenHuyen'].'</option>';
                               ...
                               echo $result;
                                ...
                                if(isset($_POST['mahuyen']))
                                {
                                    $xa=$_POST['mahuyen'];
                                    $value="";
                                    $diachixa=mysqli_query($conn,"SELECT *FROM `tbl_xa` WHERE (`maHuyen`='$xa')");
                                    if(mysqli_num_rows($diachixa)>0)
                                    {
                                        while($row_xa=mysqli_fetch_array($diachixa))
                                        {
                                            $value.='<option>'. $row_xa['tenXa'].'</option>';
                                        }
                                    }
                                    echo $value;
                                }
